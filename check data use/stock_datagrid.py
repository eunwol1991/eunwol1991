...
def load_stock(file):
    ...
    # ---- 搜索各种写法的 Product Code 列 ----
    normalized = {
        re.sub(r"\s+", "", c).lower(): c
        for c in df.columns
    }
    code_col = normalized.get("productcode")
    if not code_col:
...
with tab_sales:
    ...
    # -------- Step-3 / 4  年份 & 月份 --------
        ...
        if desc_sel == ALL:
            ...
            asc2 = st.radio(
                "Date 排序", ["升序", "降序"], key="sale_sort_dir", horizontal=True
            ) == "升序"

            data_rows2 = tbl.drop(index="总计")
            sorted_rows2 = data_rows2.sort_values("Date", ascending=asc2)
