...
def ensure_reasonable_chapters(chapters: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    ...
    first_title = chapters[0][0].strip() if chapters else ''
    if (
        len(chapters) == 1
        and first_title in ('正文', '前言')
        ...
        chapters = [(f"第{i}章", seg) for i, seg in enumerate(segs, 1)]
  ...
  <rootfiles>
    <rootfile full-path='OEBPS/content.opf' media-type='application/oebps-package+xml'/>
        ...
        if cover_info:
            ...
            cover_title = fix_named_entities(html.escape(title))
            cover_author = fix_named_entities(html.escape(author))
            cover_xhtml = (
                f"<?xml version='1.0' encoding='utf-8'?>\n<html xmlns='http://www.w3.org/1999/xhtml' xml:lang='{lang}' lang='{lang}'>\n"
                "<head>\n  <meta charset='utf-8'/><title>封面</title><link rel='stylesheet' type='text/css' href='style.css'/></head>\n"
                f"<body>\n  <h1 style='text-align:center;margin-top:3em;'>{cover_title}</h1>\n  <h3 style='text-align:center;margin-top:2em;'>{cover_author}</h3>\n</body>\n</html>"
            )
            epub.writestr('OEBPS/cover.xhtml', cover_xhtml)
            manifest.append("<item id='cover' href='cover.xhtml' media-type='application/xhtml+xml'/>")
            spine.append("<itemref idref='cover' linear='yes'/>")
