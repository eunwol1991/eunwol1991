...
class PDFExcelApp:
    ...
    def create_widgets(self):
        ...
        ttk.Label(self.root, text="选择文件夹:", font=self.font).grid(row=0, column=0, padx=10, pady=10, sticky='w')
        ...
        self.browse_button = ttk.Button(self.root, text="浏览", command=self.browse_directory, bootstyle=PRIMARY)
        self.browse_button.grid(row=0, column=2, padx=10, pady=10, sticky='ew')

        ttk.Label(self.root, text="选择输出文件夹:", font=self.font).grid(row=1, column=0, padx=10, pady=10, sticky='w')
        ...
        self.browse_output_button = ttk.Button(self.root, text="浏览", command=self.browse_output_directory, bootstyle=PRIMARY)
        self.browse_output_button.grid(row=1, column=2, padx=10, pady=10, sticky='ew')

        ttk.Label(self.root, text="选择操作:", font=self.font).grid(row=2, column=0, padx=10, pady=10, sticky='w')
        ...
        self.option_menu = ttk.Combobox(self.root, textvariable=self.option_var, values=self.options, state='readonly', font=self.font)
        self.option_menu.grid(row=2, column=1, padx=10, pady=10, sticky='ew')

        ttk.Label(self.root, text="起始数字序号:", font=self.font).grid(row=3, column=0, padx=10, pady=10, sticky='w')
        self.start_num_var = StringVar()
        self.start_num_entry = ttk.Entry(self.root, textvariable=self.start_num_var, font=self.font)
        self.start_num_entry.grid(row=3, column=1, padx=10, pady=10, sticky='ew')

        ttk.Label(self.root, text="结束数字序号:", font=self.font).grid(row=4, column=0, padx=10, pady=10, sticky='w')
        self.end_num_var = StringVar()
        self.end_num_entry = ttk.Entry(self.root, textvariable=self.end_num_var, font=self.font)
        self.end_num_entry.grid(row=4, column=1, padx=10, pady=10, sticky='ew')

        ttk.Label(self.root, text="关键词:", font=self.font).grid(row=5, column=0, padx=10, pady=10, sticky='w')
        self.keyword_var = StringVar()
        self.keyword_entry = ttk.Entry(self.root, textvariable=self.keyword_var, font=self.font)
        self.keyword_entry.grid(row=5, column=1, padx=10, pady=10, sticky='ew')

        style = ttk.Style()
        style.configure('TButton', font=self.font)

        self.convert_do_button = ttk.Button(self.root, text="Convert DO Excel to PDF", command=self.run_convert_do, bootstyle=SUCCESS, style='TButton')
        self.convert_do_button.grid(row=6, column=0, padx=10, pady=10, sticky='ew')

        self.convert_inv_button = ttk.Button(self.root, text="Convert INV Excel to PDF", command=self.run_convert_inv, bootstyle=SUCCESS, style='TButton')
        self.convert_inv_button.grid(row=6, column=1, padx=10, pady=10, sticky='ew')

        self.merge_inv_button = ttk.Button(self.root, text="Merge INV PDFs", command=self.run_merge_inv, bootstyle=SUCCESS, style='TButton')
        self.merge_inv_button.grid(row=7, column=0, padx=10, pady=10, sticky='ew')

        self.merge_do_button = ttk.Button(self.root, text="Merge DO PDFs", command=self.run_merge_do, bootstyle=SUCCESS, style='TButton')
        self.merge_do_button.grid(row=7, column=1, padx=10, pady=10, sticky='ew')
    ...
    def run_convert_do(self):
        ...
        if choice == "1":
            self.run_in_background(convert_all_do_excels, directory, output_directory, excel_files)
        elif choice == "2":
            start_num = int(self.start_num_var.get())
            end_num = int(self.end_num_var.get())
            self.run_in_background(convert_range_do_excels, directory, output_directory, start_num, end_num, excel_files)
        else:
    ...
    def run_convert_inv(self):
        ...
        choice = self.option_var.get().split('.')[0]
        if choice == "1":
            self.run_in_background(self.convert_inv_with_debug, convert_all_inv_excels, directory, output_directory, excel_files)
        elif choice == "2":
            start_num = int(self.start_num_var.get())
            end_num = int(self.end_num_var.get())
            self.run_in_background(self.convert_inv_with_debug, convert_range_inv_excels, directory, output_directory, start_num, end_num, excel_files)
        elif choice == "3":
            keyword = self.keyword_var.get()
            self.run_in_background(self.convert_inv_with_debug, convert_keyword_inv_excels, directory, output_directory, keyword, excel_files)
        else:
            messagebox.showerror("错误", "无效的选项")
    ...
    def run_merge_inv(self):
        ...
        if choice == "2":
            ...
            numbers = [int(num.strip()) for num in self.start_num_var.get().split(',')]
