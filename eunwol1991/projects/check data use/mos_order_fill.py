...
def _pick_existing_path(candidates: list[str]) -> Path:
...
PDF_DIR = _pick_existing_path(
    [
        _from_c("Users/jhunj/Dropbox/for jj/mos_pdfs"),
        _windows_path_to_wsl(_from_c("Users/jhunj/Dropbox/for jj/mos_pdfs")),
        _from_c("Users/jhunj/Dropbox/for jj/mos_pdfs"),
    ]
)  # 待处理 PDF 文件夹
PO_COL = "AI"
...
ARCHIVE_DIR = _pick_existing_path(
    [
        _from_c("Users/jhunj/Dropbox/for jj/mos order"),
        _windows_path_to_wsl(_from_c("Users/jhunj/Dropbox/for jj/mos order")),
        _from_c("Users/jhunj/Dropbox/for jj/mos order"),
    ]
)
...
def _suggest_store_candidates(
    pdf_store_line: str, store_row_keys: list[str], limit: int = 3
) -> list[str]:
...
def _suggest_item_candidates(
    item_desc: str, item_col_keys: list[str], limit: int = 3
) -> list[str]:
    if not item_desc:
        return []
...
def _fuzzy_match_item(item_desc: str, item_norm_map: dict[str, str]) -> dict | None:
    ...
    if (
        best_key
        and best_score >= ITEM_FUZZY_MIN
        and (best_score - second_score) >= ITEM_FUZZY_GAP
    ):
...
LINE_ITEM_RE = re.compile(rf"^\s*\d+\s+(.+?)\s+(\d+)\s+({UNIT_RE})\b", re.I)
            ...
            for lines in pages:
                        ...
                        not_found_store.append(
                            {
                                ...
                                "reason": reason,
                                "candidates": _suggest_store_candidates(
                                    store_line, store_row_keys
                                ),
                                "mos_lines": [
                                    ln for ln in lines if "mos" in ln.lower()
                                ][:3],
                            }
                        )
                    continue
                ...
                for it in info["items"]:
                    ...
                    if not c:
                        ...
                        if (
                            suggestions
                            and INTERACTIVE_ITEM_SELECT
                            and norm_desc not in declined_item_keys
                        ):
                        ...
                        not_found_item.append(
                            {
                                ...
                                "suggestions": _suggest_item_candidates(
                                    item_std, item_col_keys
                                ),
                            }
                        )
                        continue

                    ws.cell(r, c).value = _format_qty_uom(it["qty"], it["uom"])
