    ...
    try:
        for root, _, files in os.walk(directory):
            for file in files:
                ...
                if (
                    filename_substring.lower() in low
                    and low.endswith((".xlsx", ".xlsm", ".xlsb", ".xls"))
                    ...
                    try:
                        ...
                        is_abr = "abr" in low
                        targets = abr_targets if is_abr else normal_targets

                        if debug:
                            print("📌 文件类型：ABR" if is_abr else "📌 文件类型：普通")
                        ...
                        # 最后停留在 Invoice 界面
                        if "invoice" in sheet_map:
                            sheet_map["invoice"].activate()
                            wb.app.api.ActiveWindow.ScrollColumn = 1
                            wb.app.api.ActiveWindow.ScrollRow = 1

                            invoice_focus = targets["invoice"][1]
                            sheet_map["invoice"].range(invoice_focus).select()

                            if debug:
                                print(f"🪄 已停留在 INVOICE，并选中 {invoice_focus}")

                        elif "do" in sheet_map:
                            sheet_map["do"].activate()
                            wb.app.api.ActiveWindow.ScrollColumn = 1
                            wb.app.api.ActiveWindow.ScrollRow = 1

                            do_focus = targets["do"][1]
                            sheet_map["do"].range(do_focus).select()

                            if debug:
                                print(f"🪄 已停留在 DO，并选中 {do_focus}")
...
class App(tk.Tk):
    def __init__(self):
        ...
        ttk.Checkbutton(
            frm, text="显示 Excel 窗口", variable=self.visible_var
        ).grid(row=3, column=0, sticky="w", pady=(8, 0))

        ttk.Checkbutton(
            frm, text="打印调试日志", variable=self.debug_var
    ...
    def run(self):
        ...
        filename_substring = self.sub_var.get().strip() or "xx26"
